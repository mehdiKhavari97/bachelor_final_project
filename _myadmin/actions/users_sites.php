...
<div class="pagetitle">اجاره کنندگان</div>
	...
	<form action="" method="get">
		...
		<input type="text" name="search" value="<?php echo $searchtext; ?>" placeholder="جستجوی کاربر..." class="input_small" style="width: 200px; float: right;">
		<input type="submit" value="جستجو" class="input_small_submit" style="margin-right:5px;">
	</form>
	<!-- search -->

	
<a href="./?action=users_sites&addnew" style="float: left; margin-right: 10px;" class="a_button_blue"> + اضافه نمودن کاربر جدید</a>
<!--<a href="./excel_mobiles.php" style="float: left;" class="a_button_red"> دانلود موبایل کل کاربران</a>-->
...
if (isset($_GET['addnew']))
{
	...
	<form method="post" class="regform" action="">
		<div class="pagetitle">اضافه نمودن کاربر جدید</div>
		<input type="text" name="flname" placeholder="نام و نام خانوادگی کاربر" autocomplete="off" />
		...
		<input type="submit" name="addnewsubmit" value="ثبت کاربر جدید" />
	</form>
	<br style="clear:both;">
	<br style="clear:both;">
	';
	...
	{
		$insert = mysqli_query($dbc, 'insert into users_sites (flname,username,password) values
			(
...
if (isset($_GET['edit']))
{
	$res_edit = mysqli_query($dbc, 'select * from users_sites where id='.intval($_GET['edit']).';');
	...
	if ($num_edit==1) {
		...
		<br style="clear:both;">
		<form method="post" class="regform" action="">
			<div class="pagetitle">ویرایش کاربر</div>
			<input type="text" name="flname" placeholder="نام و نام خانوادگی کاربر" value="'.$row_edit['flname'].'" autocomplete="off" />
			<input type="text" name="username" placeholder="Username..." value="'.$row_edit['username'].'" dir="ltr" autocomplete="off" />
			<input type="submit" name="addnewsubmit" value="ویرایش کاربر" />
		</form>
		<br style="clear:both;">
		<br style="clear:both;">
		';
...
// change pass
if (isset($_GET['pass'])) {
	$res_pass = mysqli_query($dbc, 'select * from users_sites where id='.intval($_GET['pass']).';');
	...
	if ($num_pass==1) {
		...
		<br style="clear:both;">
		<form method="post" class="regform" action="">
			<div class="pagetitle">تغییر رمز کاربر</div>
			<input type="text" name="flname" placeholder="نام و نام خانوادگی کاربر" value="'.$row_pass['flname'].'" autocomplete="off" disabled />
...
<table class="tablelist" cellpadding="1" cellspacing="1" style="width: 100%;">
	...
	<?php
	$res_list = mysqli_query($dbc, 'select * from users_sites '.$saltsearch.' order by id desc limit '.$first.','.$max.';');
	...
	for ($i=0; $i < $num_list; $i++) {
		...
		<tr>
			...
			<td align="right" style="line-height:18px;">
				<b>'.$row_list['flname'].'</b>
				<br>
				<font style="font-size:11px;">نام کاربری : '.$row_list['username'].'</font><br>';
			...
			<td align="center"><a href="./?action=users_sites&page='.@$_GET['page'].'&pass='.$row_list['id'].'"><img src="./images/icons/password.png" class="table_icon" /></a></td>
			<td align="center"><a href="./?action=users_sites&page='.@$_GET['page'].'&edit='.$row_list['id'].'"><img src="./images/icons/edit.png" class="table_icon" /></a></td>
			<td align="center"';
					if ($row_list['status']==1) {echo ' style="background:#deecd9;"';}
					if ($row_list['status']==0) {echo ' style="background:#fcc;"';}
			echo '>
					';
					if ($row_list['status']==1) {echo '<a href="./?action=users_sites&disable='.$row_list['id'].'"><img src="./images/icons/enable.png" class="table_icon" /></a>';}
					if ($row_list['status']==0) {echo '<a href="./?action=users_sites&enable='.$row_list['id'].'"><img src="./images/icons/disable.png" class="table_icon" /></a>';}
			echo '</td>
			<td align="center"><a href="./?action=users_sites&delete='.$row_list['id'].'" onclick="return confirmLinkDropDB()" onclick="return confirmLinkDropDB()"><img src="./images/icons/delete.png" class="table_icon" /></a></td>
		</tr>
		';
	}
	?>
...
<?php
/////////////////////// Pages ...
$queryall = 'select count(id) from users_sites '.$saltsearch.';';
...
if($numall>$max)
{
	...
	{
		echo '<a href="./?action=users_sites&search='.urlencode($searchtext).'&page=1" class="pagenum">1</a><div style="float:left; line-height:28px;">&nbsp;...&nbsp;</div>';
	}
	...
	for($i=0;$i<$num;$i++)
	{
		...
		echo ' href="./?action=users_sites&search='.urlencode($searchtext).'&page='.$p.'" class="pagenum">'.$p.'</a>';
	}
	if(ceil($numall/$max)>6 and $page<(ceil($numall/$max)-2)){echo '<div style="float:left; line-height:28px;">&nbsp;...&nbsp;</div>';}
	if(ceil($numall/$max)>5){
		echo '<a href="./?action=users_sites&search='.urlencode($searchtext).'&page='.ceil($numall/$max).'" class="pagenum"';
...
{
	$delete = mysqli_query($dbc, 'delete from users_sites where id='.intval($_GET['delete']).';');
	if ($delete)
	{
...
{
	$enable = mysqli_query($dbc, 'update `users_sites` SET status=1 where id='.intval($_GET['enable']).';');
	if ($enable)
	{
...
{
	$disable = mysqli_query($dbc, 'update `users_sites` SET status=0 where id='.intval($_GET['disable']).';');
	if ($disable)
	{
...
{
	$enablevip = mysqli_query($dbc, 'update `users_sites` SET vip=1 where id='.intval($_GET['enablevip']).';');
	if ($enablevip)
	{
		$_SESSION['msg1'] = 'با موفقیت فعال گردید.';
		header("Location: ".$_SERVER['HTTP_REFERER']);
...
{
	$disablevip = mysqli_query($dbc, 'update `users_sites` SET vip=0 where id='.intval($_GET['disablevip']).';');
	if ($disablevip)
	{
		$_SESSION['msg1'] = 'با موفقیت غیر فعال گردید.';
		header("Location: ".$_SERVER['HTTP_REFERER']);
