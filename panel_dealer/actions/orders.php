...
<div class="pagetitle">سفارشات</div>
					...
					// settlement_request
					// settlement_request
					if (isset($_GET['settlement_request']))
					{
						$res_order = mysqli_query($dbc, 'select * from orders where 
							id='.intval($_GET['settlement_request']).'
							and settlement=0
							and payment_status=1
							and dealerid='.$row_admin['id'].';');
						...
						if($num_order==1)
						{
							$row_order = mysqli_fetch_array($res_order);
								...
								// update
								$update = mysqli_query($dbc, 'update orders set settlement=1 where id='.$row_order['id'].';');
								if ($update)
								{
									$_SESSION['msg1'] = 'درخواست تسویه با موفقیت ثبت گردید';
...
<table cellpadding="1" cellspacing="1" class="tablelist" width="90%" style="margin-right: auto; margin-left: auto;">
	...
	<?php
	$res_orders = mysqli_query($dbc, 'select * from orders where dealerid='.$row_admin['id'].' order by id desc limit '.$first.','.$max.';');
	$queryall = 'select count(id) from orders where dealerid='.$row_admin['id'].';';
	...
	for ($i=0; $i < $num_orders; $i++) { 
		...
		<tr>
			...
			if ($row_orders['payment_status']==1)
			{
				...
				if($row_orders['settlement']==0)
				{
					echo '<br><a href="./?action=orders&settlement_request='.$row_orders['id'].'" class="a_button_blue" style="font-size:10px; padding: 3px 7px; height:18px; line-height:18px;">درخواست&zwnj;تسویه</a>';
				}
				elseif($row_orders['settlement']==1)
				{
					echo '<br><font color="#aaa" style="font-size:10px;">درانتظار&zwnj;تسویه</font>';
				}
