...
<table class="tablelist" cellpadding="1" cellspacing="1" style="width: 100%;">
	...
	<?php
	$res_list = mysqli_query($dbc, 'select * from cars where dealerid="'.$row_admin['id'].'" order by id desc limit '.$first.','.$max.';');
...
<?php
/////////////////////// Pages ...
$queryall = 'select count(id) from cars where dealerid="'.$row_admin['id'].'";';
...
{
	$delete = mysqli_query($dbc, 'delete from cars where id='.intval($_GET['delete']).' and dealerid="'.$row_admin['id'].'";');
	if ($delete)
	{
...
{
	$enable = mysqli_query($dbc, 'update `cars` SET status=1 where id='.intval($_GET['enable']).' and dealerid="'.$row_admin['id'].'";');
	if ($enable)
	{
...
{
	$disable = mysqli_query($dbc, 'update `cars` SET status=0 where id='.intval($_GET['disable']).' and dealerid="'.$row_admin['id'].'";');
	if ($disable)
	{
