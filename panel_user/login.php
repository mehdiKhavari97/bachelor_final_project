...
<body>
	...
	<div id="content">
		<div class="padding20">
			<div id="tabmenu">
				<a href="./login"<?php if(!isset($_GET['register'])) { echo ' class="active"';} ?>>ورود اجاره کننده</a>
				<a href="./register"<?php if(isset($_GET['register'])) { echo ' class="active"';} ?>>ثبت نام اجاره کننده</a>
			</div>
			<br style="clear: both;">
			<?php
			// msg 1
						...
						else
						{
								...
								else
								{
									...
									else
									{
											...
											$insert = mysqli_query($dbc, 'insert into users_sites 
											(
												username,
												address,
												password,
...
?>
					...
					<form action="" method="post" class="loginform" style="padding-top: 9px;">
						...
						<br>
						<br>
						<input type="submit" value="ورود به پنل اجاره کننده" name="submitloginbox">
					</form>
					<br style="clear: both;">
					...
					if(isset($_POST['submitloginbox']) and strlen(stristr($_SERVER['HTTP_REFERER'],$url))!=0)
					{
						...
						if($num_p==1)
						{
							...
							if( password_verify($_POST['passwordpanel'],$row_p['password']) )
							{
								...
								// ok ok ok ok
								// ok ok ok ok
								$_SESSION['panel_user'] = sha1($row_p['username']);
								$_SESSION['panel_pass'] = sha1($row_p['password']);
